    	...
    	if(!Vehicule.validePlaque(plaque)) {
    		...
    		if(vehiculeId == -1) {
    			try {
                    ...
                    String requete = "insert into clientpossedevehicule (idClient, idVehicule, dateAjoutVehicule, possedeTemporairement) values ("+this.id+","+vehiculeId+",\""+timestamp+"\",0)";
					System.out.println(requete);
                    insertLienVehicule.executeUpdate(requete);
    			...
    			try {
					...
					if(!rs.isBeforeFirst()) {
						...
						java.util.Date date = new java.util.Date();
						Timestamp timestamp = new Timestamp(date.getTime());
						insertLienVehicule.executeUpdate("INSERT INTO clientpossedevehicule(idClient, idVehicule, dateAjoutVehicule, possedeTemporairement) VALUES ("+this.id+","+vehiculeId+",\""+timestamp+"\",0)");
					}else {
